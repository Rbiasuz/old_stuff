...
def sentiment_analyzer_scores_with_evaluation(sentence):
    score = analyser.polarity_scores(sentence)
    positivo = score.get('pos')
    negativo = score.get('neg')
...
#df['sentiment'] = df['Answers'].apply(lambda x: sentiment_analyzer_scores_with_evaluation(x))
